...
var Message = function(config, single, plural){
  ...
  return {
    ...
    sendToGroup: function(sender_fullname, recipient_id, recipient_fullname, text, sender_id, attributes, type, metadata, timestamp){
      ...
      winston.debug("Rollout-node-sdk message.sendToGroup", message);
     return new Promise(function(resolve, reject){
      ...
      var admintoken = "";
       ...
       request.post('/' + plural + admintoken, message).then(function(data){
        winston.debug("Rollout-node-sdk message.sent", data);
         resolve(data);
       }).catch(function(err){
        winston.error("Rollout-node-sdk message.sent error", err);
         reject(err);
       });
     });
   },
